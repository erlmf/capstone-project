...
def build_category_trend_payload(df: pd.DataFrame, granularity: str) -> dict:
    ...
    if category_summary:
        ...
        highest_discount = max(category_summary, key=lambda x: x["avg_discount"])
        highest_revenue  = max(category_summary, key=lambda x: x["total_revenue"])
        highlights = [
            {"title": "Kategori dengan promo paling intens",           "category": highest_promo["category"],
             "value": f"{highest_promo['avg_promo_rate'] * 100:.0f}%",
        },
            {"title": "Kategori dengan rata-rata diskon tertinggi",         "category": highest_discount["category"],
             "value": f"{highest_discount['avg_discount'] * 100:.0f}%",
             },
            {"title": "Kategori dengan revenue tertinggi", "category": highest_revenue["category"],
             "value": "Rp" + f"{highest_revenue['total_revenue']:,.0f}".replace(",", "."),
             "detail": "Revenue tertinggi berdasarkan total revenue pada periode ini."},
        ]
...
class AIInsightRequest(BaseModel):
    context: str       # "overview" | "analisis" | "simulator"
    data: dict = {}
...
def ai_insight(
    ...
    (Overview / Analisis / Simulator). Berbeda dari /ai-recommendation yang
...
def run_simulate(
    ...
    if not ds.did_simulator_result:
        raise HTTPException(status_code=400, detail="Jalankan Analisis terlebih dahulu.")
    ...
    return {
        ...
        "sku_a_result": {
            "baseline_qty": round(float(baseline_qty_a), 1),
            "baseline_rev": round(float(baseline_rev_a), 0),
            "qty_new":      round(float(qty_a_new), 1),
            "rev_new":      round(float(rev_a_new), 0),
            "qty_uplift_pct":     round(float(uplift_pct) * 100, 2),
            "rev_uplift":   round(float(rev_a_uplift), 0),
        ...
        "summary": {
            ...
            "analisis_timestamp":       ds.did_simulator_result.get("timestamp", "unknown"),
