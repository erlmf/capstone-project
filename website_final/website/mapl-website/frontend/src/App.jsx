    ...
    <aside style={{ width: 240, background: '#0F2D6B', display: 'flex', flexDirection: 'column', height: '100vh', position: 'sticky', top: 0, fontFamily: 'Poppins, sans-serif', flexShrink: 0 }}>
      ...
      <div style={{ padding: '10px 12px' }}>
        <div style={{ background: SB.profileBg, borderRadius: 14, padding: '10px 12px' }}>
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            {[['Dataset', datasets.length], ['Analisis', totalAnalyzed],].map(([label, val]) => (
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '0 10px 6px' }}>
        ...
        {datasets.map((d) => {
          ...
          const hasAnalisis = d.has_cannibalization || d.has_forecast
          return (
            <div
              key={d.dataset_id}
              onClick={() => onSelect(d)}
            ...
            >
              ...
              <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 4, flexShrink: 0 }}>
                {hasAnalisis && <span style={{ width: 6, height: 6, borderRadius: '50%', background: '#4ade80' }} />}
                <button
...
// Overview page
// ─────────────────────────────────────────────────────────────────────────
function OverviewPage({ active, checks, datasetId, onNav }) {
      ...
      {!trendsLoading && trends && (
        <>
          {(trendPayload.highlights || []).length > 0 && (
            <Card className="py-4">
              <h3 className="text-sm font-semibold text-brand-blue mb-3">Highlights Kategori</h3>
              <div className="grid grid-cols-3 gap-3">
                {trendPayload.highlights.map((item, i) => (
                  <div key={i} className="border border-brand-sky/20 rounded-xl px-4 py-4 bg-gray-50/50">
                    <p className="text-xs text-gray-400 mb-2 whitespace-nowrap">{item.title}</p>
                    <div className="flex items-center justify-between gap-3">
                      <h3 className="text-sm font-semibold text-brand-blue truncate">{item.category}</h3>
                      <span className="text-lg font-bold text-brand-blue flex-shrink-0">{item.value}</span>
                    </div>
                  </div>
            ))}
          </div>
        ...
        cachedText={active?.context_insights?.overview}
        ready={!trendsLoading}
      />
...
// ─────────────────────────────────────────────────────────────────────────
function AnalisisPage({ datasetId, onResult }) {
  const [loading, setLoading]               = useState(false)
  ...
  useEffect(() => {
    ...
    api.getDatasetResults(datasetId)
      .then(data => {
        ...
        setCachedInsight(data.context_insights?.analisis || null)
      })
  ...
  if (!result && !loading) return (
    <Card className="text-center py-16">
      ...
      <h3 className="text-base font-semibold text-brand-blue mb-2">Cannibalization Analisis</h3>
  ...
  if (loading) return <Card><Spinner label="Menjalankan Analisis..." /></Card>
    ...
    <div className="space-y-6">
      ...
      {chartData.length > 0 && (
        <Card>
          ...
          <p className="text-xs text-gray-400 mb-4">berdasarkan rata-rata revenue harian SKU (Victim)</p>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-xs text-gray-400 border-b border-brand-sky/20">
                ...
                <th className="py-2 pr-4 font-medium">Produk Diskon</th>
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex-shrink-0 w-28 text-right">
                  ...
                  <p className={`text-xs font-bold ${result.sku_a_result.rev_uplift >= 0 ? 'text-green-600' : 'text-red-500'}`}>
                    {fmt(result.sku_a_result.rev_uplift)}
                  </p>
                </div>
              </div>
            </div>
...
function AIInsightsPage({ datasetId, hasCannibalization, hasForecast }) {
  ...
  const [error, setError]     = useState(null)
  const formatRevSaved = (val) => {
  if (!val) return '—'
  const str = String(val).trim()
  
  // handle format M (juta): 5.7M, 5,7M, Rp5.7M, Rp5,7M
  const mMatch = str.match(/([0-9]+[.,][0-9]+|[0-9]+)\s*[Mm]/)
  if (mMatch) {
    const num = parseFloat(mMatch[1].replace(',', '.')) * 1e6
    return `Rp${Math.round(num).toLocaleString('id-ID')}`
  }
  
  // handle format K (ribu): 133K, Rp133K
  const kMatch = str.match(/([0-9]+[.,][0-9]+|[0-9]+)\s*[Kk]/)
  if (kMatch) {
    const num = parseFloat(kMatch[1].replace(',', '.')) * 1e3
    return `Rp${Math.round(num).toLocaleString('id-ID')}`
  }

  // handle format Jt/jt (juta Indonesia): 5,7jt, Rp5.7jt
  const jtMatch = str.match(/([0-9]+[.,][0-9]+|[0-9]+)\s*[Jj][Tt]/)
  if (jtMatch) {
    const num = parseFloat(jtMatch[1].replace(',', '.')) * 1e6
    return `Rp${Math.round(num).toLocaleString('id-ID')}`
  }

  // fallback: strip semua non-angka
  ...
  if (!recs && !rawText && !loading) return (
    <div className="space-y-6">
      <div className="grid grid-cols-4 gap-4 opacity-40 pointer-events-none">
        ...
        <StatCard label="Potensi Revenue Saved per Bulan" value="—" />
      ...
      <Card className="text-center py-12">
        ...
        {!ready && <p className="text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-xl p-3 mb-4 max-w-sm mx-auto">Jalankan <strong>Analisis</strong> dan <strong>Forecasting</strong> terlebih dahulu.</p>}
    ...
    return (
      <div className="space-y-6">
        <div className="grid grid-cols-3 gap-4">
          ...
          <StatCard label="Revenue Saved per Bulan" value={formatRevSaved(stats.potential_revenue_saved) || '—'} />
...
export default function App() {
  ...
  const [analisisFlags, setAnalisisFlags] = useState({ cannibalization: false, forecast: false, didSimulator: false })
  ...
  function handleUploaded(data) {
    ...
    setAnalisisFlags({ cannibalization: false, forecast: false, didSimulator: false })
    setNav('overview')
  }

  function handleDeleted(id) {
    setDatasets(prev => prev.filter(d => d.dataset_id !== id))
    if (active?.dataset_id === id) { setActive(null); setNav('overview') }
  }
  ...
  const breadcrumbMap = {
    overview: ['Overview', active?.filename].filter(Boolean),
    analisis:  ['Overview', 'Analisis'],
    simulator: ['Overview', 'Simulator'],
    ai:        ['Overview', 'AI Insights'],
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex font-sans">
      ...
      <main className="flex-1 px-8 py-8 w-full">
        <Header breadcrumbs={breadcrumbMap[nav] || ['Overview']} active={active} onExport={handleExport} onNav={setNav} />

        {nav === 'overview' && <OverviewPage active={active} checks={activeChecks} datasetId={active?.dataset_id} onNav={setNav} />}
        {nav === 'analisis'  && active && <AnalisisPage   datasetId={active.dataset_id} onResult={markActiveDataset} />}
        {nav === 'simulator' && active && <SimulatorPage  datasetId={active.dataset_id} hasCannibalization={analisisFlags.cannibalization} />}
        {nav === 'ai'        && active && <AIInsightsPage datasetId={active.dataset_id} hasCannibalization={analisisFlags.cannibalization} hasForecast={analisisFlags.forecast} />}
        {nav === 'settings'  && <SettingsPage username={username} />}
        {!active && !['overview', 'settings'].includes(nav) && (
          <Card className="text-center py-16">
